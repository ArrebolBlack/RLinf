...
The ``GpuNativeMultiExportBackendEnv`` composition uses one batch-size-one
engine per lane when a phase requires different reset factors.
...
class GpuNativeBackendEnv:
    ...
    def __init__(
        ...
        *,
        ...
        self._observation_track = (
            ...
            else selected_observation_track
        )
    ...
    def reset(self, requests: Any) -> tuple[Any, ...]:
        ...
        if self._planner is not None:
            ...
            self._active_requests = requests_tuple
    ...
    def step(
        ...
        *,
        ...
        if not explicit_mask and all(result is not None for result in results):
    ...
    def _step_commands(self, commands: Any) -> tuple[Any, ...]:
        ...
        if len(selected) != self._num_envs:
            raise ValueError("GPU-native step requires one command per lane")
        results = self.step(selected)
        if any(result is None for result in results):
            raise GpuNativeBackendUnavailableError(
                "GPU-native full-batch Planner step lost lane results"
            )
        typed_results = tuple(results)
        self._last_results = typed_results
    ...
    def planner_tape(self) -> tuple[tuple[dict[str, Any], ...], ...]:
        ...
        if self._planner is None:
            return self._generic_planner_tape()
    ...
    def replay_planner_tape_diagnostic(
        ...
        if isinstance(tape, GpuNativePlannerTape):
            return self._replay_generic_planner_tape_diagnostic(tape)
        if self._planner is None:
            raise GpuNativeBackendUnavailableError("online Planner mode is not enabled")
    ...
    def _generic_planner_tape(self) -> GpuNativePlannerTape:
    ...
    def _replay_generic_planner_tape_diagnostic(
        self,
